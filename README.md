...
| `REAL_DEBRID_API_KEY` | '' | `your_private_token` | Optional Real-Debrid token. When set, each Jackett result includes an RD playback URL and a labeled magnet-copy stream. |
...
Real-Debrid no longer provides a reliable global instant-availability lookup to addons, so `RD+` is intentionally learned from successful resolutions rather than guessed. Every RD result also has a `[Magnet]` copy that retains its torrent hash and trackers, so Stremio can offer its magnet-copy action independently of Real-Debrid availability. Tokens placed in configured manifest URLs are Base64-encoded, not encrypted; environment configuration is safer for a personal deployment.
